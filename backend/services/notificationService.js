...
function getTransporter() {
  ...
  if (process.env.EMAIL_HOST && process.env.EMAIL_USER && process.env.EMAIL_PASS) {
    ...
    console.log('[NOTIFICATION] Email transporter configured');
  } else {
    console.log('[NOTIFICATION] No email credentials — using ethereal test account');
  }
  return transporter;
}
...
async function sendEmail({ to, subject, html }) {
  ...
  try {
    ...
    await transport.sendMail({ from: `"SOU Examination" <${from}>`, to, subject, html });
  ...
  try {
    ...
    for (const student of students) {
      ...
      const result = await sendEmail({
        to: student.email,
        subject: `📝 Exam Reminder: ${exam.subjectName} — Tomorrow at ${formatTime(exam.time)}`,
        html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f8faf9;border-radius:12px">
          ...
          <div style="background:white;padding:24px;border-radius:12px;border:1px solid #D9E2DC">
            <h3 style="color:#176B3A;margin:0 0 16px">📝 Exam Tomorrow!</h3>
            <p style="color:#1F2933;margin:8px 0">Dear <strong>${student.name}</strong>,</p>
            <p style="color:#1F2933;margin:8px 0">This is a reminder that you have an exam scheduled tomorrow.</p>
...
async function notifyResultPublished(exam) {
  ...
  try {
    ...
    if (!submissions.length) return { sent: 0, total: 0 };
    ...
    return { sent, total: submissions.length };
  } catch (err) {
